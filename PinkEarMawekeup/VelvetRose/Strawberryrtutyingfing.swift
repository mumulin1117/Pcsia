...
class Strawberryrtutyingfing: UIViewController {
    ...
    @IBOutlet weak var postshongbgF: UIImageView!
    ...
    override func viewDidLoad() {
        ...
        if let covert = changeDescibeDatPEA["PEApublishPics"]?.components(separatedBy: "()").first {
            ...
            postshongbgF.isHidden = true
            realshongbgPEADF.isHidden = true
            intoPOSTbPEa.isHidden = true
        }
        if WisteriaWash.unniqiePEa.relationdataYuangzuPEA.PEAFollowings.filter({ dicv in
    ...
    @IBAction func reporthangePagePEAchn(_ sender: Any) {
        ...
        let sijfer1 = lilaclusterngCharhing.rosewatreFabricDNA(encryptedSilhouette: "Rwemppobrvtf yonrs kBvlborcakc?f$b$e$d$m$nAfrsez tynohuy hsiurrfep lygotuy tweaqnktr ottow yrlefpsoerftq vonra sbgliaqcekglqivsrtq stchwiesm fubsveery?u dAcfotjemre dtyhheb lubsieprn bizsi gbnlaakcskmlbiusvthepdr,f lazlxll drueblyastfemdd ziwngfmoarumkaqtsigognj hwqiilklg cnpot gluocnzgfeere hbieo hddiyslpdlraqyheqds$v$e$a$i$yRzelpdocrotz$s$v$v$u$fBnljojcjkc$o$s$g$x$eBrliobcikg usnufcfcwensjsxegde!").components(separatedBy: "$$$$$")
        ...
        let baozhenPEAACtion = CDAlertViewAction(title: sijfer1[3],textColor: .white,backgroundColor: .black) { cd in
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1.5, execute: DispatchWorkItem(block: {
                ...
                sucirAlert.show()
                WisteriaWash.unniqiePEa.removePEAUSetuser(depthlongPEA:pinkfont,getinguserIDPEA: self.changeDescibeDatPEA,browsePEA:twinklingDream)
    ...
    @IBAction func prefyourAttitudePEac(_ sender: UIButton) {
        ...
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1.0, execute: DispatchWorkItem(block: {
            ...
            if WisteriaWash.unniqiePEa.relationdataYuangzuPEA.PEAFollowings.filter({$0["PEAusernID"] == self.changeDescibeDatPEA["PEAusernID"]}).count == 0 && sender.isSelected == true{
                ...
                if counPink >= 10 &&  buidingsPEA.count > 2{
                    WisteriaWash.unniqiePEa.relationdataYuangzuPEA.PEAFollowings.insert(self.changeDescibeDatPEA, at: 0)
                }
                
            }
            
            if WisteriaWash.unniqiePEa.relationdataYuangzuPEA.PEAFollowings.filter({$0["PEAusernID"] == self.changeDescibeDatPEA["PEAusernID"]}).count >= 1 && sender.isSelected == false{
                for (iooo,Item) in WisteriaWash.unniqiePEa.relationdataYuangzuPEA.PEAFollowings.enumerated() {
                    if Item["PEAusernID"] == self.changeDescibeDatPEA["PEAusernID"] {
                        ...
                        if counPink >= 10 &&  buidingsPEA.count > 2{
                            WisteriaWash.unniqiePEa.relationdataYuangzuPEA.PEAFollowings.remove(at: iooo)
                        }
    ...
    @IBAction func todetailPostpublishPEA(_ sender: Any) {
        ...
        let descibePEAVC = CupcakeCopper.init(depthlongPEA:pinkfont,changeDescibeDatPEA:changeDescibeDatPEA ,browsePEA:twinklingDream)
